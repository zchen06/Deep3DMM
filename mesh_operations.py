...
def generate_transform_matrices(mesh, factors=None, n_verts_list=None):
    ...
    if n_verts_list is not None:
        for i,n_verts in enumerate(n_verts_list):
            ds_f, ds_D = qslim_decimator_transformer(M[-1], n_verts_desired=n_verts)
            D.append(ds_D.tocoo())
            ...
            print('decimation %d by number of verts %.0f finished' %(i,n_verts))
